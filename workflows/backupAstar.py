...
def plotPointsInLine(coords, _color = "blue"):
    xs = [x[0] for x in coords]
    ys = [x[1] for x in coords]
    plt.plot(xs, ys, '-', color = _color, linewidth = 2)
    plt.plot(xs, ys, '.', color = _color, linewidth = 2)
...
def run_astar(map_path, mask_path, coord_list: List[Tuple[float, float]], alt_list: List[Tuple[float, float]]):
    ...
    def astar(_coord_list: List[Tuple[float, float]], _alt_list: List[Tuple[float, float]]):
        ...
        logging.info("running A* algorithm...")             # Hell begins.
        ...
        for p in _coord_list:
            ...
            # p1 = (int(p[1]/16), int(p[0]/16))               # Downscale particle by 1/16th coordinates to match map downscaling
            nnd_obj = [(p[0], p[1]), (0,0), 0, 0, 0]        # Create blank object to store value we want to push to the exported DataFrame
            # ^^^ Particle, Landmark, G* Dist, A* Dist, Smooth Dist
            path_coords = [0, 0]                            # ^^^ Same as above but with path coordinates
            ...
            for j in ordered_alt_list:
                ...
                if p1 != p2:            # Make sure landmark and particle aren't the same...
                    ...
                    i_start = (_Xi[0], _Yi[0])  # Set a value for start of our line
                    i_end = (_Xi[-1], _Yi[-1])  # Set another value for the end of our line

                    if(i_end == p1 or i_start == p1):
                    # if(getDist(i_end, p1) <= 10 or getDist(i_start, p1) <= 10):
                        print(f"{i_end} , {p1} , {getDist(i_end, p1)}")
                        print(f"{i_start} , {p1} , {getDist(i_start, p1)}")
                    # Our intitial landmark line has found our particle
                    # WOOH! Less work for the program!!
                        print("OneLine")

                        _Y = _Yi        # Set our main coordinate set to the coordiantes we just found
                        _X = _Xi        # Again here...

                        dist = len(_Y)  # Calculate distance
                    else:
                        ...
                        # If our particle is outside of the mask area...
                            # print("Outside")
                            _Xii, _Yii = makeHolePath(flippedMap, 1, (p1[0], p1[1]), (p2[0], p2[1]), (max_len - 1))
                            ...
                            _XYii = np.column_stack((_Xii,_Yii))
                            for x in _XYii:
                                c_map = cv2.circle(c_map, x, 1, (0, 0, 0), -1)
                    ...
                    '''try:
                        mids, smoothDist = points2line(_X, _Y)
                        lm_mids, lm_smoothDist = points2line(_Xi, _Yi)
                        oob_mids, oob_smoothDist = points2line(_Xii, _Yii)
                        smoothDist = smoothDist + lm_smoothDist + oob_smoothDist
                        
                        plotPointsInLine(mids, "blue")
                        plotPointsInLine(lm_mids, "red")
                        plotPointsInLine(oob_mids, "green")
                        mids = mids + lm_mids + oob_mids
                        print("Outside Mask")
                    except:
                        try:
                            mids, smoothDist = points2line(_X, _Y)
                            lm_mids, lm_smoothDist = points2line(_Xi, _Yi)
                            smoothDist = smoothDist + lm_smoothDist
                    ...
                    print(f"Goldstar Distance: {j[2]}, A* Distance: {dist}, Smooth Distance:{smoothDist}")
                    print(f"TIME: {timeSince} / {curTime} Particle: {i_l} - Landmark: {i_j}")

                    xs = [x[0] for x in mids]
                    ys = [x[1] for x in mids]

                    # dpi = fig.dpi
                    if(dist < j[2] or smoothDist < j[2]):
                        fig = plt.figure()
                        plt.scatter(_X, _Y)
                        plotPointsInLine(mids, "blue")
                    ...
                    # plt.savefig(f"./NewSmooth_Out/{i_l}_{i_j}.png", dpi=dpi*2)
                    # plt.savefig(f"./LOBF_Out/{i_l}_{i_j}.png", dpi=dpi*2)

                    if dist < small_dist:           # If we find a new smallest distance...
                        print("New Smallest Dist!") # Dopamine boost!
                        ...
                        # Updating our arrays:
                        nnd_obj[1], nnd_obj[2], nnd_obj[3], nnd_obj[4] = (p2[0]*16, p2[1]*16), j[2]*16, small_dist*16, smoothDist*16
                        path_coords[0], path_coords[1] = xs, ys
    ...
    clean_real_df[['og_coord', 'astar_coord', 'goldstar_dist', 'astar_dist', 'smoothed_dist']] = pd.DataFrame(
