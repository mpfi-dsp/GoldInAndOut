...
def run_clust_area(df, pb, real_coords, rand_coords, img_path, distance_threshold=120, n_clusters="None",
                   affinity='euclidean', linkage='ward', rad=30):
    """
    ...
    og_img = img_og.copy()
    for coord_list in [real_coords]:
        for c in coord_list:
            new_img = img_og.copy()
            x, y = int(c[1]), int(c[0])
            og_img = cv2.circle(og_img, (x, y), radius=rad, color=(0, 255, 0))
            new_img = cv2.circle(new_img, (x, y), radius=rad, color=(0, 255, 0))

        cv2.imwrite('test.png', og_img)
        print('save')
